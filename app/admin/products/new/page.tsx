...
export default function NewProductPage() {
  ...
  const [form, setForm] = useState({
    ...
    collection_id: '', fabric: '100% Combed Cotton', gsm: '240 GSM', fit: 'Oversized',
    print_type: 'Screen Print', wash_care: 'Machine wash cold, inside out. Do not bleach.',
    published: true, featured: false, new_arrival: true, bestseller: false, trending: false,
    badge: '', tags: '',
  });
  const [variants, setVariants] = useState(
    SIZES.map(size => ({ size, color: 'Black', sku: '', stock: '0' }))
  );
  ...
  async function handleSave() {
    ...
    try {
      ...
      const imageData = images
        .filter(url => url.trim())
        .map((url, i) => ({ product_id: product.id, image_url: url.trim(), display_order: i }));
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8 lg:py-12">
        <div className="flex items-center gap-3 mb-8">
          ...
          <div>
            ...
            <h1 className="text-2xl lg:text-3xl font-black uppercase tracking-tight text-[#0a0a0a]">Add Product</h1>
          </div>
        </div>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white border border-gray-100 p-6 space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div className="space-y-1">
                ...
                <input type="text" value={form.slug} onChange={e => { setSlugEdited(true); updateForm('slug', e.target.value.toLowerCase().replace(/[^a-z0-9-]/g, '')); }} className="w-full border border-gray-200 px-3 py-2.5 text-sm outline-none focus:border-[#0a0a0a]" />
              </div>
              <div className="space-y-1">
          ...
          <div className="bg-white border border-gray-100 p-6 space-y-4">
            ...
            <div className="space-y-2">
              ...
              <button onClick={() => setImages(prev => [...prev, ''])} className="text-xs text-[#c9a84c] flex items-center gap-1 hover:underline">
          ...
          <div className="flex gap-3">
            <button
              onClick={handleSave}
              disabled={saving}
              className="bg-[#0a0a0a] text-white px-8 py-3 text-xs uppercase tracking-widest font-semibold hover:bg-[#c9a84c] hover:text-[#0a0a0a] transition-colors disabled:opacity-50 flex items-center gap-2"
            >
